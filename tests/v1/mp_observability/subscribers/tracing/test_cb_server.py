...
class TestCBV3RootSpanClose:
    ...
    def test_v3_root_closes_without_store_final(self, exporter):
        ...
        for et, md in [
            ...
            bus.publish(Event(event_type=et, session_id=sid, metadata=md))
        time.sleep(0.3)
        bus.stop()
    ...
    def test_retrieve_noop_emits_point_span_under_root(self, exporter):
        ...
        bus.start()
        sid = "cb-noop-span"
        bus.publish(Event(event_type=EventType.CB_REQUEST_START, session_id=sid))
        bus.publish(
            Event(
                event_type=EventType.CB_RETRIEVE_NOOP,
                session_id=sid,
                metadata={"reason": "beyond_slot_bound", "dropped_matches": 4},
            )
        )
        bus.publish(Event(event_type=EventType.CB_REQUEST_END, session_id=sid))
        time.sleep(0.3)
        bus.stop()
