...
V3 has no post-inference store_final — the request ends at
``CB_RETRIEVE_END`` — so its ``CB_RETRIEVE_SUBMITTED`` carries
``expects_store_final=False`` and the store-final bridge stays empty; the root
close is gated on ``_pending_gpu_ops`` alone.  The sentinel still matters
there: at TP>1 each worker retrieves, and a worker whose retrieve is a no-op
publishes ``CB_REQUEST_END`` on the CPU while another worker's scatter is
still on the stream.
...
class BlendTracingSubscriber(EventSubscriber):
    ...
    def _on_submitted(self, event: Event) -> None:
        ...
        unless it says ``expects_store_final=False`` (V3, which ends the request
        at ``CB_RETRIEVE_END``; marking it would leave the root span open
        forever).  ``CB_STORE_FINAL_SUBMITTED`` clears that marker (store_final
        has arrived).
    ...
    def _on_point(self, event: Event) -> None:
        ...
        Args:
            event: ``CB_FINGERPRINTS_REGISTERED``, ``CB_CHUNKS_EVICTED``, or
                ``CB_RETRIEVE_NOOP``.  The first two are published from the
                fingerprint/eviction paths, which run outside the originating
                request's span, so they routinely land as trace roots of their
