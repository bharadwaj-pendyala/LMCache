...
# Cap on outstanding START timestamps awaiting their END.  A CB request can be
# abandoned mid-lookup (the poll loop just stops re-issuing), so unmatched
# STARTs are normal; evict the oldest instead of growing without bound.
...
class BlendMetricsSubscriber(EventSubscriber):
    ...
    V3 phase metrics — the sub-legs of the unified lookup and of the retrieve
    scatter (see ``docs/design/v1/mp_observability/blend_v3_observability.md``).
    Each ``*_duration`` pairs the phase's START/END events by session, so the
    numbers match the same-named trace spans:
    ...
    - ``lmcache_blend.retrieve_noops``               — retrieves that scattered
      nothing, labeled by ``reason``.  Every one silently degrades its request to
      a full recompute, so this is the "CB engaged but was not faster" signal.
    """

    # Phase name per START / END event.  Both directions of the same phase must
    # map to the same name; that name is the histogram suffix and the trace-span
    # basename.
    ...
    def _on_phase_end(self, event: Event) -> None:
        ...
        if dt_ms < 0:
            # GPU-callback events are timestamped from the stream, so a
            # start/end pair can invert against a CPU-timestamped partner.
            return
