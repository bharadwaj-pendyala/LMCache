...
**Metrics.** The existing `lmcache_blend.*` counters stay (`lookup_requests`,
`lookup_hit_tokens`, `lookup_storage_hits`, `lookup_stale_chunks`,
`retrieve_requests`, `retrieve_failures`, `chunks_evicted`, …). Every sub-span
event above also feeds a metric, so the phase breakdown survives trace sampling:
a `*_duration` histogram per leg (`fingerprint_match`, `prefix_lookup`,
`coordinator_match`, `sparse_prefetch`, `scatter`, plus `lookup` and `retrieve`
themselves) and counters for their payloads (`fingerprint_matches`,
`coordinator_matches`, `coordinator_match_timeouts`, `sparse_prefetch_l2_keys`,
`sparse_prefetch_found_keys`, `scatter_tokens`, `scatter_prefix_chunks`,
`scatter_shifted_chunks`, `scatter_dropped_chunks`). See
[METRICS.md](METRICS.md) § *CB V3 Phase Metrics*.

Three V3-specific wiring points that are easy to get wrong:

- **No-op retrieves.** A retrieve that drops all its matches returns *success*,
  so no failure counter moves and the request silently full-recomputes.
  `CB_RETRIEVE_NOOP` → `lmcache_blend.retrieve_noops{reason}` is the only signal;
  `reason` must stay a fixed code, never an interpolated value, because it is a
  metric attribute.
- **`CB_RETRIEVE_SUBMITTED`.** V3 must publish it (with
  `expects_store_final=False`) even though it has no store_final: at TP>1 a
  worker whose retrieve is a no-op publishes `CB_REQUEST_END` on the CPU while
  another worker's scatter is still on the stream, which would otherwise close
  `cb.request` early. The V2 store-final bridge stays inert under V3.
- **The V2-only store counters** (`store_pre_computed_*`, `store_final_*`) do not
  populate under V3: its store goes through `LMCacheDrivenTransfer`, already
  observed by the MP `mp.store` span and its metrics. What V3 adds on top —
  fingerprint registration — is `CB_FINGERPRINTS_REGISTERED`, published by the
  queue drainers once the matcher has indexed the chunks (so the count reflects
  chunks actually matchable, and the `session_id` is the enqueuing store's
  request, not whichever request drained the queue).
